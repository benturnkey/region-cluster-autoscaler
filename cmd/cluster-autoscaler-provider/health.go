...
func newRouterHTTPServer(addr string, reporter providerHealthReporter) *http.Server {
	...
	mux.HandleFunc("/healthz", func(w http.ResponseWriter, _ *http.Request) {
		...
		if _, err := fmt.Fprintf(
			...
			reporter.ConfiguredProviderCount(),
			reporter.HealthyProviderCount(),
			reporter.UnhealthyProviderCount(),
		); err != nil {
			klog.Errorf("failed to write health response: %v", err)
		}
	})
	mux.HandleFunc("/ready", func(w http.ResponseWriter, _ *http.Request) {
		if reporter.HealthyProviderCount() < 1 {
